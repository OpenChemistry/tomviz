...
public:
  ...
  void SetParentCamera(vtkCamera* c)
    {
      this->ParentCamera = c;
    }

  void SetChildCamera(vtkCamera* c)
    {
      this->ChildCamera = c;
    }
  ...
  m_visible(false)
{
  ...
  // Measurement cube
  {
    ...
    m_volumeScaleRep->SetMaxRelativeCubeScreenArea(.001);
  ...
  m_renderer->SetViewport(0.85, 0.05, 1.0, 0.25);
  ...
  m_linkCamerasId = renderView->GetActiveCamera()->
    AddObserver(vtkCommand::ModifiedEvent, m_linkCameras.Get());
...
void ScaleLegend::dataSourceAdded(DataSource* ds)
{
  ...
  QObject::connect(ds, SIGNAL(dataPropertiesChanged()),
