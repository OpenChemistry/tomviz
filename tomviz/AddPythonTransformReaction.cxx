...
OperatorPython* AddPythonTransformReaction::addExpression(DataSource* source)
{
      ...
      scriptLabel == "Connected Components")
  {
    ...
    {
      labelDescription = new QLabel("Threshold image. Voxels with values between minimum and maximum intensities\n"
  ...
  else if (scriptLabel == "Pad Volume")
  {
    ...
    {
      QMap<QString, QString> substitutions;
      substitutions.insert("###padWidthX###",
                           QString("padWidthX = (%1,%2)").arg(padSizeBeforeX->value()).arg(padSizeAfterX->value()));
      substitutions.insert("###padWidthY###",
                           QString("padWidthY = (%1,%2)").arg(padSizeBeforeY->value()).arg(padSizeAfterY->value()));
      substitutions.insert("###padWidthZ###",
                           QString("padWidthZ = (%1,%2)").arg(padSizeBeforeZ->value()).arg(padSizeAfterZ->value()));
      substitutions.insert("###padMode_index###",
  ...
  else if (scriptLabel == "Reconstruct (Constraint-based Direct Fourier)")
  {
    ...
    //Description
    QLabel *label = new QLabel(
                               "Reconstruct a tilt series using constraint-based Direct Fourier method. The tilt axis must be parallel to the x-direction and centered in the y-direction. The size of reconstruction will be (Nx,Ny,Ny).\n"
    ...
    label = new QLabel("1. Apply a Fourier Gaussian filter with sigma = maximum spatial frequency *");
    ...
    if (dialog.exec() == QDialog::Accepted)
    {
      ...
      substitutions.insert("###Niter_update_support###",
                           QString("Niter_update_support = %1").arg(supportUpdateIteration->value()));
      substitutions.insert("###supportSigma###",
                           QString("supportSigma = %1").arg(supportSigma->value()));
      substitutions.insert("###supportThreshold###",
