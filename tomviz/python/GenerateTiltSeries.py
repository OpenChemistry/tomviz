def transform_scalars(dataset):
    ...
    Ny = volume.shape[1];
    Nz = volume.shape[2];
    N = np.round(np.sqrt(Ny**2 + Nz**2)) #calculate the size s.t. it contains the entire volume
    ...
    volume_pad = np.lib.pad(volume, ((0, 0), (pad_y_pre, pad_y_post), (pad_z_pre, pad_z_post)), 'constant')
    ...
    for i in range(Nproj):
        ...
        rotatedVolume = scipy.ndimage.interpolation.rotate(volume_pad, angles[i], axes=(1, 2), reshape=False, order=1)
