...
class liveTomoOperator(tomviz.operators.CancelableOperator):
	...
	def transform(self,dataset,hostName=None,userName=None,password=None,
                  port=None,filename=None,remoteDirectory=None,localDirectory=None):

		# localDirectory = '/Users/jonathanschwartz/Desktop/'
		...
		while True:
			...
			if newVolume:
				try: 
					...
					if firstLoad: recon = np.zeros(file['recon'].shape, dtype=np.float32); firstLoad = False
					recon[:,:,:] = file['recon'][:,:,:]
