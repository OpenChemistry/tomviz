...
def tv_minimization(A,tiltSeries,recon,iterNum=1):
    ...
    for i in range(iterNum): #main loop
        ...
        for j in range(0,ng):
            ...
            v1d = np.sqrt(1e-8+(r-np.roll(r,1,axis=0))**2+(r-np.roll(r,1,axis=1))**2+(r-np.roll(r,1,axis=2))**2)
            ...
            v3d = np.sqrt(1e-8+(np.roll(r,-1,axis=1)-np.roll(np.roll(r,-1,axis=1),1,axis=0))**2
                          +(np.roll(r,-1,axis=1)-r)**2
                          +(np.roll(r,-1,axis=1)-np.roll(np.roll(r,-1,axis=1),1,axis=2))**2)

            v4n = r-np.roll(r,-1,axis=2)
            v4d = np.sqrt(1e-8+(np.roll(r,-1,axis=2)-np.roll(np.roll(r,-1,axis=2),1,axis=0))**2
