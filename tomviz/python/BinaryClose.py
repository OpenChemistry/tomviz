...
class BinaryClose(tomviz.operators.Operator):

    def transform(self, dataset, structuring_element_id=0, radius=1,
                  object_label=1, background_label=0):
        """Perform morphological closing (a dilation followed by an
        erosion) on segmented objects with a given label by a spherically
        symmetric structuring element with a given radius. Fills holes and
        gaps smaller than the element."""
        import numpy as np
        ...
        kept = ndimage.binary_erosion(grown, structure=structure,
                                      border_value=1)
        result[grown & ~kept] = background_label
