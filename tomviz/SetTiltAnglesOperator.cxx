...
class SetTiltAnglesWidget : public tomviz::EditOperatorWidget
{
  Q_OBJECT
public:
  SetTiltAnglesWidget(tomviz::SetTiltAnglesOperator *op, vtkSmartPointer<vtkDataObject> dataObject, QWidget *p)
    : EditOperatorWidget(p), Op(op)
  {
    ...
    for (vtkIdType i = 0; i < totalSlices; ++i)
    {
      ...
      // deliberate non-else-if.  We want to override with the previous value from
      // the operator, but we need the previousTiltAngles array set correctly, so the
    ...
    if (this->tabWidget->currentIndex() == 0)
    {
      ...
      else
      {
        ...
        for (int i = 0; start + i <= end; ++i)
        {
          ...
          this->tableWidget->item(i, 0)->setData(Qt::DisplayRole, QString::number(tiltAngles[start + i]));
        }
      }
...
SetTiltAnglesOperator::SetTiltAnglesOperator(QObject *p)
  : Operator(p)
{
}
