/*
    Transparent conversion for dense and sparse Eigen matrices

    This function is adapted from the pybind11 code by Wenzel Jacob.
...
struct EigenProps
{
  ...
  static constexpr auto descriptor =
    ...
    // For a reference type (e.g. Ref<MatrixXd>) we have other constraints that
    // might need to be satisfied: writeable=True (for a mutable reference),
    // and, depending on the map's stride options, possibly f_contiguous or
    // c_contiguous.  We include them in the descriptor output to provide some
    // hint as to why a TypeError is occurring (otherwise it can be confusing to
    // see that a function accepts a 'numpy.ndarray[float64[3,2]]' and an error
    // message that you *gave* a numpy.ndarray of the right type and dimensions.
